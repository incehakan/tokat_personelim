...
class MovableCountServicesScreen extends StatefulWidget {
  const MovableCountServicesScreen({
    Key? key,
  }) : super(key: key);

  @override
  State<MovableCountServicesScreen> createState() => _MovableCountServicesScreenState();
}

class _MovableCountServicesScreenState extends State<MovableCountServicesScreen> {
  @override
  void initState() {
    super.initState();
    context.read<MovableCountCubit>().getServices(
          context.read<MovableCountCubit>().selectedUnit!.id!.round().toString(),
        );
  }
    ...
    return ListView(
      children: List.generate(
        ...
        (index) => InfoCardTemplate(
          onTap: () {
            context.read<MovableCountCubit>().selectService(services[index]);
            context.pop();
          },
